...
; inputs: n -- node n
;         c -- first var in clause
;         cur -- current color index
; output: a list of clauses
(defun at-most-one-color-helper (n c cur k)
  (cond ((> cur k) nil)
        (t (cons (list (* (node2var n c k) (- 0 1)) (* (node2var n cur k) (- 0 1))) (at-most-one-color-helper n c (+ 1 cur) k)))
  )
)
...
(defun at-most-one-color (n c k)
  (cond ((OR (> c k) (= c k)) nil)
        (t (append (at-most-one-color-helper n c (+ 1 c) k) (at-most-one-color n (+ 1 c) k)))
  )
)
...
; input: x -- node that does not have the same color as y
;        y -- node
;        c -- current color index
;        k -- max color index
(defun generate-edge-clauses-helper (x y c k)
  (cond ((> c k) nil)
        (t (cons (list (* (node2var x c k) (- 0 1)) (* (node2var y c k) (- 0 1))) (generate-edge-clauses-helper x y (+ 1 c) k)))
